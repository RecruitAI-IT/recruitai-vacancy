...
deploy_to_server() {
    ...
    sshpass -p "$PROD_PASSWORD" ssh -o StrictHostKeyChecking=no root@$PROD_HOST -p 22 \
        ...
        RECRUITAI_RELEASE_TG_BOT_PREFIX="$RECRUITAI_RELEASE_TG_BOT_PREFIX" \
        bash << 'EOFMAIN'
set -e
...
update_release_status_internal() {
    ...
    local endpoint="${PROD_DOMAIN}${RECRUITAI_RELEASE_TG_BOT_PREFIX}/release"
